...
def host_routing(request):
    updated_reg = utils.healthcheck(register)
    header = request.META.get('HTTP_HOST')
    for entry in config['hosts']:
        if(header == entry['host']):
            healthy_server = utils.get_healthy_server(header,updated_reg)
            if(healthy_server):
                res = requests.get('http://{}'.format(healthy_server))
                result = json.loads(res.content.decode('utf-8'))
                return JsonResponse({'status_code':200,'content':result})
            return JsonResponse({'status_code':404,'content':'No Server Available'})
